...
//module begin
    ...
    $.fn.jqGrid = function( pin ) {
        ...
        return this.each( function() {
            ...
            var sortkeys = ["shiftKey","altKey","ctrlKey"],
                ...
                populate = function (npage) {
                    if(!ts.grid.hDiv.loading) {
                        ...
                        if($.isFunction(ts.p.resetPostData)){
                            ts.p.resetPostData.call(ts,ts.p.postData);
                        }
            ...
            if(isMSIE) { if(String(ts.p.height).toLowerCase() === "auto") { ts.p.height = "95%";$(grid.bDiv).css({"overflow":"scroll"}); } }
            $(grid.bDiv)
            ...
            //1、有总计，且有合计的时候，就在合计的table下面添加一行作为总计展示区域。
            //2、有总计、且无合计的时候，用合计的列代替总计
            if(ts.p.amount && ts.p.footers){
                var $tr = $(grid.sDiv).find("tr:first").clone(true);
                $(grid.sDiv).find("tbody").append($tr);
            }
    ...
    $.jgrid.extend({
        ...
        footerData : function(action,data, format,rowIndex) {
                ...
                $(this.p.colModel).each(function(i){
                    ...
                    if(action === "set") {
                        if( data[nm] !== undefined) {
                            ...
                            title = this.title ? {"title":$.jgrid.stripHtml(vl)} : {};
                            $("tr.footrow:eq("+rowIndex+") td:eq("+i+")",t.grid.sDiv).html(vl).attr(title);
                            success = true;
                        }
                    } else if(action === "get") {
                        res[nm] = $("tr.footrow:eq("+rowIndex+")  td:eq("+i+")",t.grid.sDiv).html();
                    }
                });
            });
        ...
        getCol : function (col, obj, mathopr,calcFn) {
                        ...
                        while(i<ln){
                            if($($t.rows[i]).hasClass('jqgrow')) {
                                ...
                                if(mathopr) {
                                    ...
                                    //单元格中数据不是纯数字的情况
                                    if(isNaN(v)){
                                        v = ($t.p.footers && $t.p.footers.format) ? $t.p.footers.format(val) :0;
                                        v = parseFloat(v);
                                    }
                                    if(!isNaN(v)) {
                                        if(calcFn){
            ...
            return this.each(function() {
                ...
                if( maxfrozen>=0 && frozen) {
                    ...
                    $($t.grid.fbDiv).on('mousewheel DOMMouseScroll', function (e) {
                        ...
                        if($t.grid.bDiv.clientHeight !== $t.grid.bDiv.scrollHeight){
                            e.preventDefault();
                        }
