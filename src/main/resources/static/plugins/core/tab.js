...
layui.define(['element', 'common','menu'], function (exports) {
    "use strict";
     ...
     * @param {String} menuId
     */
    Tab.prototype.getTabId = function (menuId) {
        var that = ELEM.titleBox === undefined ? this.init() : this,
            tabId = -1;
        ELEM.titleBox.find('li').each(function (i, e) {
            var $cite = $(this).children('cite');
    ...
    Tab.prototype.tabAdd = function (data) {
        if(data.href === "#") data.href="/vetech/notExist/";
        var that = this;
        var _config = that.config;
        ...
        if (tabIndex === -1) {
            ...
            title += '<cite data-id="'+data.id+'">' + data.title + '</cite>';
            if (_config.closed) {
            ...
            //切换到当前打开的选项卡
            element.tabChange(ELEM.tabFilter, that.getTabId(data.id));
                ...
                if ($context.length > 0) {
                    $context.eq(0).children('ul').children('li').each(function () {
                        ...
                        $that.on('click', function () {
                            ...
                            switch (target) {
                                case 'refresh': //刷新当前
                                    var currIframe =ELEM.contentBox.find('iframe[data-id=' + id + ']');
                                    if(currIframe.length){
                                        ELEM.contentBox.find('iframe[data-id=' + id + ']')[0].src = currIframe[0].src;
                                    }
                                    break;
        ...
        if (_config.onSwitch) {
            element.on('tab(' + ELEM.tabFilter + ')', function (data) {
                ...
                if(ELEM.titleBox.children('li').length>2 &&  $(this).is("li")){
                    var id = $(this).find("cite").data("id");
                    menu.setActive($("#" + id));
                }
            });
        }
    ...
    /**
     * 根据下标获取该下标所在的index
     * @param index 这里的index是从1开始的，不是从0该是
     */
    Tab.prototype.getTabIdByIndex = function(index){
        var that = this;
        var _config = that.config;
        return $(_config.elem).find('ul.layui-tab-title').children('li').eq(index).attr('lay-id') || 0;
    };
