...
;!function(){
  ...
  ,laydate2 = {
    v: '5.0.4'
    ,config: {} //全局配置项
    ,index: (window.laydate2 && window.laydate2.v) ? 100000 : 0
    ,path: ready.getPath
    ...
    ,ready: function(fn){
      ...
      ,path = (isLayui ? 'modules/laydate/' : 'theme/') + 'default/laydate.css?v='+ laydate2.v + ver;
  ...
  //字符常量
  ,MOD_NAME = 'laydate2', ELEM = '.layui-laydate', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'laydate-disabled', TIPS_OUT = '开始日期超出了结束日期<br>建议重新选择', LIMIT_YEAR = [100, 200000]
  ...
  ,Class = function(options){
    var that = this;
    that.index = ++laydate2.index;
    that.config = lay.extend({}, that.config, laydate2.config, options);
    laydate2.ready(function(){
      that.init();
    });
  }
  ...
  */
  ...
  Class.prototype.init = function(){
    ...
    //设置唯一KEY
    if(!options.elem.attr('lay-key2')){
      options.elem.attr('lay-key2', that.index);
      options.eventElem.attr('lay-key2', that.index);
    }
    ...
    that.elemID = 'layui-laydate2'+ options.elem.attr('lay-key2');
    
    if(options.show || isStatic) that.render();
    isStatic || that.events();
  };
  
  Class.prototype.updateMaxAndMin = function(){ 
  	var options = this.config;
  	if(options.maxId){
  		options.max = lay(options.maxId)[0].value;	
  		update(['max']);
  	}
  	if(options.minId){
  		options.min = lay(options.minId)[0].value;
  		update(['min']);
  	}
  ...
  Class.prototype.render = function(){
    ...
    ,elem = that.elem = lay.elem('div', {
      ...
      ,'class': [
        ...
        ,options.format === "HH:mm" ? ' layui-laydate-hm':''
      ].join('')
    })
  ...
  Class.prototype.checkDate = function(fn){
    ...
    ,checkValid = function(dateTime){
      ...
      //计算当前月的最后一天
      thisMaxDate = laydate2.getEndDate(dateTime.month + 1, dateTime.year);
    ...
    //add code 取最后一个日期值作为选中项
      var last = null;
    if(typeof value === 'string' && value){
        //add code 多个日期值跳过正常格式校验
        if(!options.multi){
  ...
  Class.prototype.calendar = function(value){
    ...
    prevMaxDate = laydate2.getEndDate(dateTime.month, dateTime.year); //计算上个月的最后一天
    thisMaxDate = laydate2.getEndDate(dateTime.month + 1, dateTime.year); //计算当前月的最后一天
    ...
    lay.each(tds, function(index, item){
      ...
      if(index < startWeek){
        ...
        st = index - startWeek;
        if(!options.range){
            //add code 去掉其他月份同一日期的选中效果
          st + 1 === dateTime.date && nowMonth == dateTime.month && item.addClass(THIS);
        }
      } else {
  ...
  Class.prototype.list = function(type, index){
    ...
    if(type === 'year'){ //年列表
      ...
      var tempArr = options.format === "HH:mm" ? [24,60] : [24, 60, 60];
      lay.each(tempArr, function(i, item){
  ...
  Class.prototype.parse = function(state){
    ...
    // add code 日期的选中和反选中效果
    var elm = options.elem[0];
    var splitCode = options.split||"/";
    var hasSplit = elm.value?splitCode:"";
      var clear = false;
      function clearSelect(str,newValue){
        if(!str) return newValue;
        str = str.split(splitCode);
        ...
        if(!clear) str.push(newValue);
        return str.join(splitCode);
      }
    if(options.multi){
      return clearSelect(elm.value,format.join(''));
    }else{
      return format.join('');
    }
  };
  ...
  Class.prototype.choose = function(td){
    ...
    //add code 多次选择日期的选中/反选中效果
    tds.removeClass(THIS);
    td.addClass(THIS);
    ...
    if(options.range){
      ...
      that.calendar().done().done(null, 'change');
        ...
        var choosedStr = that.parse();
        that.setValue(choosedStr).done([choosedStr]);
  ...
  Class.prototype.events = function(){
    ...
    if(!options.elem[0] || options.elem[0].eventHandler){
    	options.elem.off(options.trigger);
    }
  ...
  //核心接口
  laydate2.render = function(options){
    var inst = new Class(options);
    return thisDate.call(inst);
  };
  
  //得到某月的最后一天
  laydate2.getEndDate = function(month, year){
    var thisDate = new Date();
    //设置日期为下个月的第一天
  ...
  isLayui ? (
    ...
    ,layui.define(function(exports){ //layui加载
      laydate2.path = layui.cache.dir;
      exports(MOD_NAME, laydate2);
    })
  ) : (
